...
class JoinMarketProtocolManager(AbstractNPartyProtocol):
    ...
    def __init__(self, role, msgchan, callbackdata):
        #The handle to a persistent msgchan object is passed
        #to each instantiation of a JMProto instance, to avoid
        #having to recreate/restart for each transaction.
        ...
        self.current_event = self.get_initial_event()
        ...
        #start listening on the provided message channel.
        self.msgchan.register_channel_callbacks(
            on_welcome=self.on_welcome,
            on_set_topic=self.on_set_topic,
            on_connect=self.on_connect,
            on_disconnect=self.on_disconnect,
            on_nick_leave=self.on_nick_leave,
            on_nick_change=None, #TODO
            on_pubmsg_trigger=self.on_pubmsg,
            on_privmsg_trigger=self.on_privmsg)
    ...
    def on_pubmsg(self, nick, msg):
        ...
        parsed_msg = msg[1:].split(separator)[0]
    ...
    def on_x_offer(self, nick, msg, first_ordertype):
        ...
        for orderline in orderlines:
            ...
            self.receive_event(2, oid, ordertype, minsize, maxsize,
                               txfee, cjfee)
