...
@pytest.mark.parametrize("tx_type, tx_id, tx_hex, address", [
    ("simple-tx",
...
if not btc.secp_present:
    ...
    def donation_address(cjtx, wallet):
        ...
        log.debug("Using the following nonce value: " + binascii.hexlify(
            sign_k))
        c = btc.sha256(btc.multiply(
            binascii.hexlify(sign_k), reusable_donation_pubkey, True))
