...
def test_donation_address(setup_donations, amount):
    ...
    dest_addr = donation_address(reusable_donation_pubkey)
    print dest_addr
...
def donation_address(reusable_donation_pubkey):
    global sign_k
    #deterministic random, simple example
    sign_k = btc.sha256(reusable_donation_pubkey)
    log.debug("Using the following nonce value: " + sign_k)
